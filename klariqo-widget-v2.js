...
(function() {
  ...
  class KlariqoWidget {
    constructor() {
      ...
      this.userInitiatedClose = false; // Track if user explicitly closed widget
    ...
    toggleWidget() {
      ...
      if (this.isOpen) {
        ...
        this.userInitiatedClose = false; // Opening widget
        ...
        this.userInitiatedClose = true; // User explicitly closed widget
    ...
    connectWebSocket() {
      ...
      if (this.ws && this.ws.readyState !== WebSocket.CLOSED) {
        console.log('[Klariqo] Closing existing WebSocket before reconnecting');
        this.ws.close();
      }
      ...
      this.ws.onclose = (event) => {
        console.log(`[Klariqo] WebSocket closed - code: ${event.code}, reason: ${event.reason}`);
        ...
        // Auto-reconnect if NOT user-initiated close and widget is still open
        if (!this.userInitiatedClose && this.isOpen && this.reconnectAttempts < this.maxReconnectAttempts) {
          this.reconnectAttempts++;
          console.log(`[Klariqo] 🔄 Auto-reconnecting... (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`);

          setTimeout(() => {
            if (this.isOpen && !this.userInitiatedClose) {
              this.connectWebSocket();
            }
          }, 1000);
        } else if (this.reconnectAttempts >= this.maxReconnectAttempts) {
          console.error('[Klariqo] ❌ Max reconnect attempts reached');
          const status = document.getElementById('klariqoStatus');
          if (status) {
            status.textContent = 'Connection lost. Please close and reopen.';
          }
        }
      };
    }
