 ...
 */ function getCurrentDateTimeContext(client) {
 ...
 */ export function buildVoiceOptimizedPrompt(client, voiceProfile) {
  const accentLabel = voiceProfile.accent === 'US' ? 'American' : voiceProfile.accent === 'UK' ? 'British' : 'Australian';
  const channelContext = client.channel_type === 'phone' ? 'You are speaking with a customer who called our business for help.' : client.channel_type === 'website' ? 'You are speaking with a visitor on our website who initiated a chat.' : 'You are speaking with a customer who either called us or is chatting on our website.';
  ...
  if (client.tone) {
    const toneGuidance = client.tone === 'professional' ? 'Maintain a professional, courteous tone.' : client.tone === 'friendly' ? 'Be warm, friendly, and approachable - like talking to a friend.' : client.tone === 'casual' ? 'Keep it casual and relaxed - no need to be overly formal.' : client.tone === 'technical' ? 'Use industry-specific terminology when appropriate, be precise and detailed.' : 'Maintain a balanced, professional tone.';
...
- "tomorrow" = ${new Date(now.getTime() + 86400000).toLocaleDateString('en-CA', {
      timeZone: timezone
    })}
- "today" = ${dateStr}
...
Business Hours:
${Object.entries(client.business_hours || {}).map(([day, hours])=>{
    if (hours.closed) {
